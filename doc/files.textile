...
for greater flexibility in this regard.  Multi-unit files should only be used when required by the language or when the
types are sufficiently related that placing them in a single file promotes maintainability.

h2. The Standard Case

Files should contain a single top-level definition, meaning an un-nested class, trait or object. One exception to this
